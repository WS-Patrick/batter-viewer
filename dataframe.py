...
def dataframe(level, player_id):
    ...
    df=pd.DataFrame(raw, columns = ['game_id','pitch_type', 'game_date', 'release_speed', 'release_pos_x', 'release_pos_z',  'player_name', 'batname', 'batter', 'pitcher', 'events', 'description', 'zone', 'des', 'stand', 'p_throw', 'pitcherteam','batterteam', 'home_team' , 'away_team',
...
def base_df(player_df):
    ...
    inplay = pd.pivot_table(player_df, index='game_year', values='inplay', aggfunc='count', margins=False)
...
def pivot_base_df(player_df, pivot_index):
    ...
    inplay = player_df.groupby(['game_year']).apply(lambda x: x.pivot_table(index=pivot_index, values='inplay', aggfunc='count', margins=True))
